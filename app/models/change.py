...
class Change:
    ...
    Attributes:
        ...
        category: Category of change (endpoint, method, parameter, schema, response, request_body)
        ...
        details: Optional additional details about the change (e.g., location, old/new values)
    """
    type: Literal["breaking", "potentially_breaking", "non_breaking"]
    category: Literal["endpoint", "method", "parameter", "schema", "response", "request_body"]
    path: str
...
class DiffResult:
    ...
    def __str__(self) -> str:
        ...
        if self.old_version or self.new_version:
            lines.append(f"Comparing: {self.old_version or 'unknown'} → {self.new_version or 'unknown'}")
            lines.append("")
        
        lines.append(f"Total changes: {len(self.changes)}")
        lines.append(f"Breaking: {self.summary.get('breaking', 0)}")
        lines.append(f"Potentially breaking: {self.summary.get('potentially_breaking', 0)}")
